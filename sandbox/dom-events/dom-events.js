...
function renderTasks(tasks) {
    ...
    taskList.innerHTML = "";
    const listEntries = tasks.map(taskTemplate).join("");
    taskList.innerHTML = listEntries;
  // get the list element from the DOM
  // loop through the tasks array. transform (map) each task object into the appropriate HTML to represent a to-do.
}

function newTask() {
    let entry = document.querySelector("#todo").value;
    tasks.push({ detail: entry, completed: false });
    renderTasks(tasks);
    entry.value = "";
